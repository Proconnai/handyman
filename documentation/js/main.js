...
function documentReadyInit() {
	...
	if (jQuery().tweet) {
		jQuery('.twitter').tweet({
			...
			template: "@{user}<div class=\"tweet_right\">{join}<span class=\"tweet_text\">{tweet_text}</span>{time}</div>"
		});
	}
...
function windowLoadInit() {
	...
	if ($header.length) {
		...
		if ($header.hasClass('header_offset') && $header.hasClass('header_white')) {
			$header.parent().addClass('header_offset_wrap white_wrap');
		}
		if ($header.hasClass('header_offset') && $header.hasClass('header_darkgrey')) {
			$header.parent().addClass('header_offset_wrap darkgrey_wrap');
		}
